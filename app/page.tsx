...
export default function Home() {
  ...
  const { resolve, isResolving, error, clearError } = useMediaResolver();
  async function submit() {
    const parsed = parseYouTubeUrl(url);
    if (!parsed.valid) { clearError(); setPreview(null); return; }
    const item = await resolve({ url, format, quality });
    if (item) { setPreview(item); setQueue((current) => [...current, item]); }
  }
  function download(item: MediaItem) { window.open(item.downloadUrl, "_blank", "noopener,noreferrer"); }
  return <main className="mx-auto min-h-screen max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
    ...
    <section className="mx-auto mt-10 max-w-3xl"><UrlInput value={url} onChange={(value) => { setUrl(value); clearError(); }} onSubmit={submit} loading={isResolving} /><div className="mt-4"><QualitySelector format={format} quality={quality} onFormat={setFormat} onQuality={setQuality} /></div>{error && <p role="alert" className="mt-4 rounded-2xl bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</p>}{!isResolving && !error && url && !parseYouTubeUrl(url).valid && <p className="mt-3 px-3 text-sm text-rose-700">Enter a valid YouTube video URL to continue.</p>}</section>
    <section className="mx-auto mt-8 max-w-3xl">{isResolving && <PreviewSkeleton />}{preview && !isResolving && <VideoPreviewCard item={preview} onDownload={() => download(preview)} />}</section>
    <div className="mx-auto max-w-3xl"><BatchQueue items={queue} onDownload={download} onRemove={(index) => setQueue((items) => items.filter((_, itemIndex) => itemIndex !== index))} /></div>
